...
class SUN(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        self.phase = phase
        self.name = 'SUN_' + phase

        print('Loading SUN dataset - phase {0}'.format(phase))
        ...
        if self.phase=='train':
            ...
            self.labels = np.load(label_train_categories_train_phase)
            self.data = np.load(file_train_categories_train_phase)
            # self.image_id=np.load(ImageId_train)
            ...
            if self.phase=='test':
                ...
                data_base_labels = np.load(label_train_categories_test_phase)
                data_base_images = np.load(file_train_categories_test_phase)
                # image_id_base=np.load(ImageId_train)
                # load data that will be use for evaluating the few-shot recogniton
                # accuracy on the novel categories.
                data_novel_labels = np.load(label_test_categories_test_phase)
                data_novel_images = np.load(file_test_categories_test_phase)
                # image_id_novel=np.load(ImageId_test)
            else: # phase=='val'
                # load data that will be used for evaluating the recognition
                # accuracy of the base categories.
                data_base_labels = np.load(label_train_categories_val_phase)
                data_base_images = np.load(file_train_categories_val_phase)
                # image_id_base=np.load(ImageId_train)
                ...
                # load data that will be use for evaluating the few-shot recogniton
                # accuracy on the novel categories.
                data_novel_labels = np.load(label_val_categories_val_phase)
                data_novel_images = np.load(file_val_categories_val_phase)
                # image_id_novel=np.load(ImageId_val)

            self.data = np.concatenate(
                [data_base_images, data_novel_images], axis=0)
            self.labels = np.concatenate(
                [data_base_labels, data_novel_labels], axis=0)
            # self.image_id=np.concatenate(
            #     [image_id_base, image_id_novel], axis=0)

            self.label2ind = buildLabelIndex(self.labels)
            self.labelIds = sorted(self.label2ind.keys())
            self.num_cats = len(self.labelIds)

            self.labelIds_base = buildLabelIndex(data_base_labels).keys()
            self.labelIds_novel = buildLabelIndex(data_novel_labels).keys()
            self.num_cats_base = len(self.labelIds_base)
        ...
        mean_pix = [x/255.0 for x in [119.48502004896787, 116.7135452219681, 108.39537579902651]]

        std_pix = [x/255.0 for x in [64.57476636359503, 63.57792211582089, 69.41985574081532]]
...
class FewShotDataloader():
    ...
    def createExamplesTensorData(self, examples):
        ...
        # img_ids=torch.LongTensor([self.dataset.image_id[img_idx] for img_idx, _ in examples])
        # return images, torch.stack([labels,img_ids],dim=0)
    ...
    def get_iterator(self, epoch=0):
        ...
        tnt_dataset = tnt.dataset.ListDataset(
            elem_list=range(self.epoch_size), load=load_function)#i-th sample is returned by `load(elem_list[i])`
